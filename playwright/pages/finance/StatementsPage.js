 ...
 *
 * @typedef {'text' | 'select' | 'multiSelect' | 'dateRange'} StatementsFilterType
 */
...
export class StatementsPage {
   ...
   * @returns {Promise<number>}
   */
  async selectOneEligibleFor(chip) {
  ...
  /**
   * @param {RegExp[]} optionLabels
   * @param {number} [index]
   */
  async setRuleMultiSelect(optionLabels, index = 0) {
   ...
   * @param {number} [index]
   */
  async setRuleDateRange(fromYmd, toYmd, index = 0) {
    await this.#valueCell(index).getByRole('button').click();
