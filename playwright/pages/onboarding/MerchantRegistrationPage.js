...
/** @typedef {import('../../fixtures/onboarding/types.js').MailingAddressConfig} MailingAddressConfig */
/** @typedef {import('../../fixtures/onboarding/types.js').MerchantBusinessConfig} MerchantBusinessConfig */
/** @typedef {import('../../fixtures/onboarding/types.js').PartnerBusinessConfig} PartnerBusinessConfig */
/** @typedef {import('../../fixtures/onboarding/types.js').PaymentMethodsConfig} PaymentMethodsConfig */
/** @typedef {import('../../fixtures/onboarding/types.js').PayoutConfig} PayoutConfig */
/** @typedef {import('../../fixtures/onboarding/types.js').StakeholderConfig} StakeholderConfig */
 ...
 *
 * @typedef {object} StakeholderInput
 * @property {string} firstName
 * @property {string} lastName
 * @property {string} email
 * @property {string} phone Subscriber number — must differ from every other stakeholder's.
 * @property {string} identificationId
 * @property {boolean} ubo UBO flag — `true` ticks "Ultimate Beneficial Owner" and fills 100%
 *   ownership; `false` leaves the stakeholder a non-owner (so the drawer's
 *   `100 − others` ownership cap is never breached).
 * @property {boolean} [primary] The seeded primary stakeholder (the merchant applicant).
 *   Its email is the account-creation email — seeded server-side and rendered
 *   **disabled + prefilled** in the drawer — so the drawer fill asserts that field
 ...
 *
 * @type {Omit<StakeholderInput, 'email'>}
 */
...
export class MerchantRegistrationPage {
   ...
   *
   * @param {PartnerBusinessConfig} [data]
   */
  async fillPartnerSoleBusinessTab(data) {
    if (data !== undefined) {
   ...
   * @param {PartnerBusinessConfig} [data]
   */
  async fillPartnerIndividualBusinessTab(data) {
    if (data !== undefined) {
   ...
   * @param {PartnerBusinessConfig} data
   */
  async fillPartnerBusinessTab(data) {
   ...
   *
   * @param {MerchantBusinessConfig} [data]
   */
  async fillBusinessTab(data) {
    if (data !== undefined) {
   ...
   * @param {StakeholderConfig} [data]
   * @returns {Promise<string>}
   */
  async fillStakeholdersTabForReseller(iteration = 0, data) {
    ...
    if (data !== undefined) {
      ...
      const stakeholderWithEmail = { ...data, email: merchantEmail };
   ...
   * @param {StakeholderConfig} [data]
   */
  async fillStakeholdersTab(accountEmail, data) {
    if (data !== undefined) {
      await this.#openStakeholderDrawer();
      await this.#fillStakeholderDrawerFromConfig(data, accountEmail);
      const savedCell = data.ubo === true ? '100%' : (data.firstName ?? 'PO');
      await this.#saveStakeholderDrawer({ email: accountEmail }, savedCell);
      await this.#advance(/payout details/i);
      return;
    }
    /** @type {StakeholderInput} */
    const primary = { ...PRIMARY_STAKEHOLDER, email: accountEmail };
    await this.#openStakeholderDrawer();
   ...
   * @param {string} accountEmail
   */
  async fillStakeholdersTabWithThree(accountEmail) {
   ...
   *  are filled and document uploads are skipped.
   *
   * @param {PayoutConfig} [data]
   */
  async fillPartnerPayoutTab(data) {
    if (data !== undefined) {
   ...
   *
   * @param {PaymentMethodsConfig} [data]
   */
  async fillPaymentTab(data) {
    if (data !== undefined) {
   ...
   *  timeout).
   *
   * @param {RegExp} nextStep
   */
  async #advance(nextStep) {
    await this.clickNext();
   ...
   */
  async #selectOptionStrict(id, optionLabel) {
   ...
   * @param {{ fallbackToFirst?: boolean }} [opts]
   */
  async #selectOption(id, optionText, opts) {
    const trigger = this.page.locator(`#${id}`);
   ...
   * @param {RegExp} optionText
   */
  async #selectMultiOption(id, optionText) {
    const trigger = this.page.locator(`#${id}`);
    if (!(await trigger.isVisible().catch(() => false))) return;
    if (await trigger.isDisabled().catch(() => false)) return;
    await trigger.click();
  ...
  async #uploadFile(id) {
    ...
    if ((await input.count()) > 0) {
      await input.setInputFiles(FIXTURE_PDF).catch((err) => {
        log.warn('Upload failed', { field: id, error: err.message });
      });
    }
  }
   ...
   */
  async #pickCalendarDate(id, yearsOffset) {
    const input = this.page.locator(`#${id}`);
    if (!(await input.isVisible().catch(() => false))) return;
    await input.click();
  ...
  async #openAddStakeholderDrawer() {
    await this.page
   ...
   *
   * @param {StakeholderInput} stakeholder
   */
   ...
   *
   * @param {StakeholderInput} opts
   */
  async #fillStakeholderDrawer(opts) {
    // Particulars
   ...
   * @param {string} accountEmail
   */
  async #assertPrimaryEmailLocked(accountEmail) {
   ...
   *
   * @param {{ email: string } & Partial<StakeholderInput>} stakeholder
   * @param {string | RegExp} savedCell
   */
  async #saveStakeholderDrawer(stakeholder, savedCell) {
   ...
   * @param {string | undefined} value
   */
  async #selectIfPresent(id, value) {
    if (value === undefined) return;
    const trigger = this.page.locator(`#${id}`);
    if (!(await trigger.isVisible().catch(() => false))) return;
    if (await trigger.isDisabled().catch(() => false)) return;
   ...
   */
  async #multiSelectIfPresent(id, values) {
   ...
   *
   * @param {string} id
   * @param {DateConfig | undefined} date
   */
  async #pickDateIfPresent(id, date) {
   ...
   * @param {string | undefined} value
   */
  async #fillStakeholderPhoneIfPresent(value) {
    if (value === undefined) return;
    await this.page
      .getByPlaceholder(/enter phone number/i)
      .first()
   ...
   *
   * @param {MailingAddressConfig | null | undefined} mailing
   */
  async #fillMailingAddress(mailing) {
    if (!mailing) {
      await this.#checkBox(/same as (above|mailing|business address)/i);
      return;
    }
   ...
   * @param {MerchantBusinessConfig} data
   */
  async #fillBusinessTabFromConfig(data) {
   ...
   * @param {PartnerBusinessConfig} data
   */
  async #fillPartnerCompanyParticularsFromConfig(data) {
    await this.#selectIfPresent('companyType', data.companyType);
   ...
   * @param {PartnerBusinessConfig} data
   */
  async #fillPartnerIndividualBusinessFromConfig(data) {
    await this.#selectIfPresent('companyType', data.companyType);
    await this.#selectIfPresent('thirdPartyType', data.thirdPartyType);
    await this.#fillIfPresent('companyName', this.#applyUnique(data.companyName));
   ...
   *
   * @param {StakeholderConfig} data
   * @param {string | undefined} primaryEmail
   */
  async #fillStakeholderDrawerFromConfig(data, primaryEmail) {
   ...
   * @param {PayoutConfig} data
   */
  async #fillPayoutTabFromConfig(data) {
   ...
   * @param {PaymentMethodsConfig} data
   */
  async #fillPaymentTabFromConfig(data) {
